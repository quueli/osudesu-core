...
public class BeatmapService(ILogger<BeatmapService> logger, DatabaseService database, HttpClientService client)
{
        ...
        string? beatmapHash = null, int? beatmapId = null, int? retryCount = 1, CancellationToken ct = default)
    {
        ...
        // Ограничиваем максимальное количество попыток для предотвращения rate limiting
        ...
        while (retryCount > 0 && !linkedCts.IsCancellationRequested && !IsValidResult(beatmapSetTask))
        {
            ...
            // ИСПРАВЛЕНИЕ: Делаем только ОДИН запрос за итерацию, приоритизируя по точности
            // Приоритет: beatmapId > beatmapHash > beatmapSetId
            if (beatmapId != null)
            {
            ...
            if (!IsValidResult(beatmapSetTask) && !linkedCts.IsCancellationRequested)
            {
                ...
                if (retryCount > 0)
                {
                    // Увеличиваем задержку между попытками для снижения нагрузки
    ...
    public async Task<Result<List<BeatmapSet>, ErrorMessage>> SearchBeatmapSets(BaseSession session, int[]? rankedStatus, string mode,
