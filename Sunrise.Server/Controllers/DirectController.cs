...
public class DirectController(DirectService directService, SessionRepository sessions) : ControllerBase
{
    ...
    {
        // ВАЖНО: catboy.best забанил наш сервер из-за избыточных запросов
        // Альтернативные зеркала для beatmap:
        // 1. https://api.nerinyan.moe/d/{id} - Nerinyan API (рекомендуется)
        // 2. https://osu.direct/api/d/{id} - osu.direct
        // 3. https://beatconnect.io/b/{id} - BeatConnect
        // 4. Собственное зеркало через Observatory API

        // TODO: Реализовать ротацию зеркал или использовать собственное хранилище
        return Redirect($"https://api.nerinyan.moe/d/{id}");
    }
}
